						...
						columns : [
								...
								},
								{
									...
									formatter : function(value, row, index) {
										...
										var f = '<a class="btn btn-success btn-sm" href="#" title="Preview"  mce_href="#" onclick="preview(\''
												+ row.cid
...
function batchRemove() {
	...
	if (rows.length == 0) {
		layer.msg("Please select the deleted items");
		return;
	}
	layer.confirm("Are you sure to delete the selected '" + rows.length + "'items?", {
		btn : [ 'Ok', 'Cancel' ]
	// 按钮
	}, function() {
		var ids = new Array();
