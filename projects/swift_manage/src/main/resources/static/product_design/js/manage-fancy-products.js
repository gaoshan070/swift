jQuery(document).ready(function($) {
    ...
    var fpd_fancy_products_opts = {
        selectProduct : "Please select a Product first to assign the category!",
        nothingToExport: "This product does not contain any views!",
        noJSON: "Sorry, but the selected file is not a valid JSON object. Are you sure you have selected the correct file to import?",
	...
	$productsList.on('click', '.fpd-add-view', function(evt) {
		...
		radykalPrompt({placeholder: fpd_admin_opts.enterTitlePrompt}, function(title) {
			...
			if(title === false) {
                ...
                blockProducts();
                uploadViewOpt.viewTitle = title;
                uploadViewOpt.productID = productID;
                uploadViewOpt.listItem = $listItem;
                dialog.dialog( "open" );
				// mediaUploader = null;
                // mediaUploader = wp.media({
		         //    multiple: false,
	...
	function addView() {
	   $.ajax({
			...
			data: {
                action: 'fpd_newview',
				title: uploadViewOpt.viewTitle,
				thumbnail: uploadViewOpt.uploadFileUrl,
				product_id: uploadViewOpt.productID
			},
			type: 'post',
			dataType: 'json',
			success: function(data) {
				...
				if(data !== undefined) {
					if(data.thumbnail !== undefined) {
						uploadViewOpt.listItem.find('span:first img').attr('src', data.thumbnail);
					}
				}
				else {
					fpdMessage(fpd_admin_opts.tryAgain, 'error');
    ...
    $('.designFile').change(function(){
        var id = $(this).attr("id");
        var imgId = "img_" + id;
        var url = "/manage/product/uploadFile";
        var params = {dateTime:new Date()};
        var filename = $("#"+id).val();
        ...
        if(".png" == filetype || ".jpg" == filetype){
            $.ajaxFileUpload({
                ...
                data:params,
                secureuri:false,
                fileElementId:id,        //file的id
                dataType:"json",                  //返回数据类型为文本
                success:function(data,status){
                    ajaxResponseFilter(data,"");
                    if (data.result==0){
                        $("#"+imgId).attr("src",data.path);
                        $("#"+id).next().val(data.path);
                        uploadViewOpt.uploadFileUrl = data.path;
                        alert("上传图片成功");
                    }else {
                        alert("上传图片失败，请重试");
                    }
                ...
                error: function (data, status, e){
                    window.location.href=data.responseXML.URL;
                    alert("上传图片失败，请重试");
                }
            });
        }else{
            alert("只能上传png,jpg格式的图片")
        }
    });
